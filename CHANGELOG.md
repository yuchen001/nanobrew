...
- **CI formula benchmark is 1.84x faster weighted across the expanded set** — the latest GitHub Actions push benchmark covers `actionlint`, `fd`, `ripgrep`, `uv`, `starship`, `zoxide`, and `yq`. Verified upstream is faster for 5/7 formulae with a weighted speedup of 1.84x: `actionlint` 446 ms vs 2209 ms, `ripgrep` 319 ms vs 831 ms, `uv` 663 ms vs 1035 ms, `fd` 354 ms vs 507 ms, and `zoxide` 319 ms vs 412 ms. `starship` and `yq` remain slower and stay visible in the reporting-only benchmark.
