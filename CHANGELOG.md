...
- **Shimmed `yt-dlp` installs benchmark faster than Homebrew and prior nanobrew** — local same-path testing with Homebrew auto-update disabled measured cached reinstall of `nb install --shims yt-dlp` at 423.7 ms versus `brew install yt-dlp` at 18.901 s, or 44.61x faster. The already-installed no-op path now averages 41.4 ms over 10 runs versus Homebrew at 930.3 ms, about 22.5x faster. Compared with nanobrew v0.1.191, the already-installed path improved from 9.134 s to 39.6 ms, about 231x faster; cached reinstall of just `yt-dlp` improved from 8.445 s to 57.2 ms, about 148x faster.
