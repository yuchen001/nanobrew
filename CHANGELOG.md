...
- **Shimmed `yt-dlp` installs benchmark faster than Homebrew** — local same-path testing with Homebrew auto-update disabled measured cached reinstall of `nb install --shims yt-dlp` at 423.7 ms versus `brew install yt-dlp` at 18.901 s, or 44.61x faster. The already-installed no-op path now averages 41.4 ms over 10 runs versus Homebrew at 930.3 ms, about 22.5x faster.
