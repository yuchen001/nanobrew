...
As of 2026-04-25, verified upstream coverage started from 11/100 Homebrew formulae and 18/100 Homebrew casks in the top-100 analytics sets. The current generated registry on this branch covers 95/100 formulae and 48/100 casks. Formula coverage is 4,937,354 / 5,131,605 top-100 30-day installs (96.21%); cask coverage is 836,995 / 1,610,712 (51.96%). Treat the 18/100 cask number as the baseline for the related #251-#256 work: any resolver-class or generated-record change should include a fresh coverage run and explain whether the delta comes from new records, analytics churn, or report logic.
...
Use `scripts/seed-upstream-formulas.mjs` to find popular formula candidates programmatically from Homebrew's 30-day install-on-request analytics. The seeder first writes `homebrew_bottle` records when the Homebrew formula index has a macOS arm64 bottle with a SHA256 plus any additional supported platform bottles. Those records include revision/rebuild and dependency lists so the runtime can use the native bottle path without live Homebrew metadata. If no bottle lock is available, the seeder falls back to GitHub latest-release records when it can find a macOS arm64 archive with a GitHub SHA256 asset digest and an inferable binary path inside that archive. Archive inspection has download timeouts and a size cap so broad scans skip unsuitable payloads instead of hanging. It mirrors generated records into both `registry/upstream.json` and `src/upstream/registry_default.json` when run with `--write`.
...
The current seeders are intentionally conservative and do not imply that the top 100 can be fully covered by the current registry shape alone. A broad dry run with `--include-existing --scan 100 --limit 100` reports the current ceiling for the implemented resolver classes. The current formula ceiling is 95/100 because the remaining top-100 formula gaps are third-party tap analytics rows missing from Homebrew's core formula index. Reaching full top-100 coverage requires adding more source classes and artifact shapes, such as tap formula metadata, source-build formula support, extensionless app download format probes, platform-specific cask artifact rules, pkg-only casks, suite casks, installer-script casks, font casks, tap cask analytics metadata, and casks whose verification policy is `no_check`.
...
For `github_release` upstreams, `repo` is the `owner/name` allowlist. For `vendor_url` upstreams, `allow_domains` must list the allowed download domains. For `homebrew_bottle` upstreams, `resolved.version` and `resolved.assets` are the trust boundary; records must be generated from Homebrew bottle metadata and use `verification.sha256 = "required"`.

GitHub Release formula records must define at least one `assets` entry keyed by platform, such as `macos-arm64`, `macos-x86_64`, `linux-x86_64`, or `linux-aarch64`. Homebrew bottle formula records omit `assets` rules and instead require resolved assets. Cask records must define at least one artifact declaration.
